...
setStickyOffsets() {
  ...
  renderGoalieTable(fixedContainer, tableScrollWrapper, options = {}) {
    ...
    // --- Goalie Scroll-Tabelle (Option B: 11 gleich breite Spalten, width:100% via CSS) ---
    const goalieScrollTable = document.createElement("table");
    goalieScrollTable.className = "season-table-scroll goalie-table-scroll";
    // <colgroup> mit 11 gleich breiten Spalten (je 100/11 %) — KEINE Pixel, KEINE JS-Messung
    const goalieColGroup = document.createElement("colgroup");
    for (let i = 0; i < 11; i++) {
      const col = document.createElement("col");
      col.style.width = (100 / 11) + "%";
      goalieColGroup.appendChild(col);
    }
    ...
    displayGoalieRows.forEach((row, rowIndex) => {
      ...
      createGoalieCells(row).forEach((val, cellIndex) => {
        const td = document.createElement("td");
        td.textContent = val;
        if (cellIndex === 1) {
          td.className = "season-time-cell";
          td.dataset.goalie = row.name;
          this.attachGoalieLongPressHandler(td, row.name);
        }
    ...
    if (goalieNames.length > 0) {
      ...
      // 11-Spalten total row: Games leer, MIN=Summe, GA/SA/SV=Summen, Sv%/GAA=Ø, rest leer
      const totalCells = [
        "",                                // 0: Games (leer)
        formatMinutes(totals.minutesDec),  // 1: MIN
        totalGA,                           // 2: GA
        totalSA,                           // 3: SA
        totalSV,                           // 4: SV
        totalSvPct,                        // 5: Sv%
  ...
  attachGoalieLongPressHandler(timeCell, goalieName) {
    let pressTimer = null;
    ...
    timeCell.addEventListener("mousedown", () => {
      pressTimer = setTimeout(() => {
        this.openGoalieAddTimeDialog(goalieName);
      }, 500);
    });
    ...
    timeCell.addEventListener("touchstart", () => {
      pressTimer = setTimeout(() => {
        this.openGoalieAddTimeDialog(goalieName);
      }, 500);
    }, { passive: true });
  ...
  openGoalieAddTimeDialog(goalieName) {
    const modal = document.getElementById("addTimeModal");
    if (!modal) return;

    const goalieMinutes = Number(App.data.goalieSeasonData?.[goalieName]?.minutes || 0);
    const currentSeconds = Math.round(goalieMinutes * 60);

    document.getElementById("addTimePlayerName").textContent = goalieName;
    document.getElementById("addTimeCurrentTime").textContent = App.helpers.formatTimeMMSS(currentSeconds);
    document.getElementById("addTimeInput").value = "";

    modal.dataset.playerName = "";
    modal.dataset.goalieName = goalieName;
    modal.dataset.currentTime = currentSeconds;
  ...
  handleAddTime() {
    ...
    const goalieName = modal.dataset.goalieName;
    ...
    if (goalieName) {
      if (!App.data.goalieSeasonData) App.data.goalieSeasonData = {};
      if (!App.data.goalieSeasonData[goalieName]) {
        App.data.goalieSeasonData[goalieName] = {
          num: "",
          name: goalieName,
          games: 0,
          minutes: 0,
      ...
      App.data.goalieSeasonData[goalieName].minutes += additionalSeconds / 60;
      App.storage.saveGoalieSeasonData();
      this.closeAddTimeDialog();
      this.render();
