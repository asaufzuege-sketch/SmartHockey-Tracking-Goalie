...
App.statsTable = {
  ...
  attachTimeClickHandlers(timeTd, playerName) {
    ...
    let _timeTouchStartX = 0, _timeTouchStartY = 0, _timeTouchMoved = false;
    timeTd.addEventListener('touchstart', (e) => {
      const t = e.touches[0];
      if (t) { _timeTouchStartX = t.clientX; _timeTouchStartY = t.clientY; }
      _timeTouchMoved = false;
    }, { passive: true });
    timeTd.addEventListener('touchmove', (e) => {
      const t = e.touches[0];
      if (t && (Math.abs(t.clientX - _timeTouchStartX) > 10 || Math.abs(t.clientY - _timeTouchStartY) > 10)) {
        _timeTouchMoved = true;
      }
    }, { passive: true });
    timeTd.addEventListener('touchend', (e) => {
      ...
      if (_timeTouchMoved) { _timeTouchMoved = false; return; }
  ...
  attachValueClickHandlers() {
    this.container.querySelectorAll("td[data-player][data-cat]").forEach(td => {
      ...
      let _valTouchStartX = 0, _valTouchStartY = 0, _valTouchMoved = false;
      td.addEventListener('touchstart', (e) => {
        const t = e.touches[0];
        if (t) { _valTouchStartX = t.clientX; _valTouchStartY = t.clientY; }
        _valTouchMoved = false;
      }, { passive: true });
      td.addEventListener('touchmove', (e) => {
        const t = e.touches[0];
        if (t && (Math.abs(t.clientX - _valTouchStartX) > 10 || Math.abs(t.clientY - _valTouchStartY) > 10)) {
          _valTouchMoved = true;
        }
      }, { passive: true });
      td.addEventListener('touchend', (e) => {
        ...
        if (_valTouchMoved) { _valTouchMoved = false; return; }
  ...
  changeValue(td, delta) {
    ...
    if (cat !== "+/-") {
      App.data.statsData[player][cat] = Math.max(0, App.data.statsData[player][cat]);
    }
    ...
    document.querySelectorAll(".total-cell").forEach(tc => {
      ...
      if (cat === "+/-") {
        ...
        if (!tc.dataset.listenersAttached) {
          ...
          let lastTapTime = 0;
          let tapTimeout = null;
          let _oppTouchStartX = 0, _oppTouchStartY = 0, _oppTouchMoved = false;
          
          tc.addEventListener('touchstart', (e) => {
            const t = e.touches[0];
            if (t) { _oppTouchStartX = t.clientX; _oppTouchStartY = t.clientY; }
            _oppTouchMoved = false;
          }, { passive: true });
          tc.addEventListener('touchmove', (e) => {
            const t = e.touches[0];
            if (t && (Math.abs(t.clientX - _oppTouchStartX) > 10 || Math.abs(t.clientY - _oppTouchStartY) > 10)) {
              _oppTouchMoved = true;
            }
          }, { passive: true });
          tc.addEventListener('touchend', (e) => {
            ...
            if (_oppTouchMoved) { _oppTouchMoved = false; return; }
