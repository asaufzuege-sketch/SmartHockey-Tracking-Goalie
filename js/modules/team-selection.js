...
5. Goal Map: Punkte im Feld/Tor/Time setzen, zurück zu Game Center. Für Gegentore muss vorher oben ein Goalie ausgewählt sein.
...
Team Selection
• 3 Team-Slots verfügbar (Team 1, Team 2, Team 3)
• Team wählen oder bearbeiten (Klick auf ✏️)
• Alle Daten (Spieler, Statistiken, Saison, Goal Maps) werden pro Team getrennt gespeichert
...
🥅 Goalie Stats
Auf der Season-Seite kannst du mit dem Toggle-Button oben („Goalies" / „Players") zwischen Feldspieler- und Goalie-Tabelle wechseln. Goalie-Spalten:
• Games — Spiele mit Einsatz
• MIN — gespielte Minuten (MM:SS)
• GA — Goals Against (Gegentore)
• SA — Shots Against (Schüsse aufs Tor)
• SV — Saves (SA − GA)
• Sv% — Save Percentage (SV ÷ SA; Elite ≥ 92%)
• GAA — Goals Against Average (Gegentore pro 60 Min; Elite ≤ 2.00)
• SO — Shutouts (Spiele ohne Gegentor)
• Goal Value — invertierte Gegner-Gewichtung: Gegentore gegen schwache Gegner werden stärker gewichtet
• MVP / MVP Points — Goalie-Rangliste (Sv% × Spielzeit − Goal Value/Spiel + Einsatzbonus)

Beim „Export Season" wirst du nach der Spielzeit deiner Goalies gefragt (MM:SS; 0 oder leer = nicht gespielt). Die Werte werden automatisch aus deinen Goal Map-Ereignissen berechnet.
...
• Ein Backup enthält alle 3 Teams gleichzeitig — du musst nicht pro Team sichern.
...
Reset (irreversibel)
• Jede Seite hat einen eigenen Reset-Button — es gibt keinen globalen Reset:
• Game Center: setzt Statistiken und Spielzeiten des aktuellen Teams zurück
• Season: setzt alle Saisondaten (Skater + Goalie) des aktuellen Teams zurück
• Goal Map: setzt die Punkte des aktuellen Spiels zurück (Season Map bleibt)
• Season Map: setzt die kumulierten Punkte des aktuellen Teams zurück
• Goal Value: setzt alle Gegnernamen und Gewichtungen des aktuellen Teams zurück
• Player Selection: setzt Spielerliste und Goalies des aktuellen Teams zurück`,
...
5. Goal Map: Set points in field/goal/time, return to Game Center. For conceded goals, a goalie must be selected at the top first.
...
Team Selection
• 3 team slots available (Team 1, Team 2, Team 3)
• Choose or edit team (click on ✏️)
• All data (players, statistics, season, Goal Maps) is saved separately per team
...
🥅 Goalie Stats
On the Season page, use the toggle button at the top ("Goalies" / "Players") to switch between the skater table and the goalie table. Goalie columns:
• Games — games with deployment
• MIN — minutes played (MM:SS)
• GA — Goals Against (goals conceded)
• SA — Shots Against (shots on goal)
• SV — Saves (SA − GA)
• Sv% — Save Percentage (SV ÷ SA; elite ≥ 92%)
• GAA — Goals Against Average (goals conceded per 60 min; elite ≤ 2.00)
• SO — Shutouts (games without a goal conceded)
• Goal Value — inverted opponent weighting: goals against weak opponents are weighted more heavily
• MVP / MVP Points — Goalie ranking (Sv% × ice time − Goal Value/game + deployment bonus)

During "Export Season" you will be asked for each goalie's ice time (MM:SS; 0 or blank = did not play). The values are automatically calculated from your Goal Map events.
...
• One backup contains all 3 teams at once — you don't need to back up per team.
...
Reset (irreversible)
• Each page has its own reset button — there is no global reset:
• Game Center: resets statistics and ice times of the current team
• Season: resets all season data (skaters + goalies) of the current team
• Goal Map: resets the points of the current game (Season Map remains)
• Season Map: resets the accumulated points of the current team
• Goal Value: resets all opponent names and weights of the current team
• Player Selection: resets the player list and goalies of the current team`,
...
5. Goal Map: Установить точки на поле/воротах/времени, вернуться в Game Center. Для пропущенных голов вверху должен быть выбран вратарь.
...
Team Selection
• Доступно 3 слота для команд (Команда 1, Команда 2, Команда 3)
• Выбрать или редактировать команду (клик на ✏️)
• Все данные (игроки, статистика, сезон, Goal Maps) сохраняются отдельно для каждой команды
...
🥅 Goalie Stats
На странице Season используйте переключатель («Goalies» / «Players») для перехода между таблицей полевых игроков и таблицей вратарей. Столбцы вратарей:
• Games — игры с участием
• MIN — сыгранные минуты (ММ:СС)
• GA — Goals Against (пропущенные голы)
• SA — Shots Against (броски в створ)
• SV — Saves (SA − GA)
• Sv% — Save Percentage (SV ÷ SA; элита ≥ 92%)
• GAA — Goals Against Average (пропущенных голов за 60 мин; элита ≤ 2.00)
• SO — Shutouts (игры без пропущенных голов)
• Goal Value — инвертированное взвешивание: голы от слабых соперников весят больше
• MVP / MVP Points — рейтинг вратарей (Sv% × время − Goal Value/игра + бонус за участие)

При «Export Season» вас попросят ввести игровое время вратарей (ММ:СС; 0 или пусто = не играл). Значения рассчитываются автоматически из событий Goal Map.
...
• Один бэкап содержит все 3 команды сразу — не нужно делать резервные копии по каждой команде.
...
Reset (необратимо)
• У каждой страницы есть собственная кнопка сброса — глобального сброса нет:
• Game Center: сбрасывает статистику и время на льду текущей команды
• Season: сбрасывает все сезонные данные (полевые + вратари) текущей команды
• Goal Map: сбрасывает точки текущей игры (Season Map остаётся)
• Season Map: сбрасывает накопленные точки текущей команды
• Goal Value: сбрасывает все имена соперников и веса текущей команды
• Player Selection: сбрасывает список игроков и вратарей текущей команды`,
...
5. Goal Map: Sätt punkter i fält/mål/tid, återgå till Game Center. För insläppta mål måste en målvakt väljas uppe först.
...
Team Selection
• 3 lagplatser tillgängliga (Lag 1, Lag 2, Lag 3)
• Välj eller redigera lag (klicka på ✏️)
• All data (spelare, statistik, säsong, Goal Maps) sparas separat per lag
...
🥅 Goalie Stats
På Season-sidan kan du använda toggleknappen uppe ("Goalies" / "Players") för att växla mellan spelar- och målvaktstabellen. Målvaktskolumner:
• Games — matcher med speltid
• MIN — spelade minuter (MM:SS)
• GA — Goals Against (insläppta mål)
• SA — Shots Against (skott på mål)
• SV — Saves (SA − GA)
• Sv% — Save Percentage (SV ÷ SA; elit ≥ 92%)
• GAA — Goals Against Average (insläppta mål per 60 min; elit ≤ 2.00)
• SO — Shutouts (matcher utan insläppt mål)
• Goal Value — inverterad motståndarsviktning: mål mot svaga motståndare väger tyngre
• MVP / MVP Points — målvaktsrankning (Sv% × istid − Goal Value/match + spelbonus)

Vid "Export Season" ombeds du ange målvakternas istid (MM:SS; 0 eller tomt = spelade ej). Värdena beräknas automatiskt från dina Goal Map-händelser.
...
• En backup innehåller alla 3 lag på en gång — du behöver inte säkerhetskopiera per lag.
...
Reset (oåterkallelig)
• Varje sida har en egen återställningsknapp — det finns ingen global återställning:
• Game Center: återställer statistik och istider för aktuellt lag
• Season: återställer alla säsongsdata (skridskospelare + målvakter) för aktuellt lag
• Goal Map: återställer poängen för aktuell match (Season Map kvarstår)
• Season Map: återställer de ackumulerade poängen för aktuellt lag
• Goal Value: återställer alla motståndarnamn och vikter för aktuellt lag
• Player Selection: återställer spelarlistan och målvakter för aktuellt lag`,
...
5. Goal Map: Aseta pisteet kentälle/maaliin/aikaan, palaa Game Centeriin. Päästettyjen maalien osalta yläosasta on ensin valittava maalivahti.
...
Team Selection
• 3 joukkuepaikaa käytettävissä (Joukkue 1, Joukkue 2, Joukkue 3)
• Valitse tai muokkaa joukkuetta (napsauta ✏️)
• Kaikki tiedot (pelaajat, tilastot, kausi, Goal Maps) tallennetaan erikseen joukkueittain
...
🥅 Goalie Stats
Season-sivulla voit käyttää yläosan vaihtonappulaa ("Goalies" / "Players") vaihtaaksesi kenttäpelaaja- ja maalivahtitaulukon välillä. Maalivahdin sarakkeet:
• Games — pelit, joissa oli peliaikaa
• MIN — pelatut minuutit (MM:SS)
• GA — Goals Against (päästetyt maalit)
• SA — Shots Against (laukaukset maalille)
• SV — Saves (SA − GA)
• Sv% — Save Percentage (SV ÷ SA; eliitti ≥ 92%)
• GAA — Goals Against Average (päästetyt maalit per 60 min; eliitti ≤ 2.00)
• SO — Shutouts (pelit ilman päästettyä maalia)
• Goal Value — käänteinen vastustajapainotus: maalit heikkoja vastustajia vastaan painavat enemmän
• MVP / MVP Points — maalivahtien rankinglista (Sv% × jääaika − Goal Value/peli + peluutusbonus)

"Export Season" -toiminnon aikana sinulta kysytään jokaisen maalivahdin jääaika (MM:SS; 0 tai tyhjä = ei pelannut). Arvot lasketaan automaattisesti Goal Map -tapahtumistasi.
...
• Yksi varmuuskopio sisältää kaikki 3 joukkuetta kerralla — ei tarvitse varmuuskopioida joukkueittain.
...
Reset (peruuttamaton)
• Jokaisella sivulla on oma nollauspainike — ei ole globaalia nollausta:
• Game Center: nollaa tilastot ja jääajat nykyisestä joukkueesta
• Season: nollaa kaikki kauden tiedot (luistelijat + maalivahdit) nykyisestä joukkueesta
• Goal Map: nollaa nykyisen pelin pisteet (Season Map pysyy)
• Season Map: nollaa nykyisen joukkueen kertyneet pisteet
• Goal Value: nollaa kaikki vastustajien nimet ja painotukset nykyisestä joukkueesta
• Player Selection: nollaa pelaajalistauksen ja maalivahdit nykyisestä joukkueesta`,
...
5. Goal Map: Placer des points sur le terrain/but/temps, retourner à Game Center. Pour les buts encaissés, un gardien doit être sélectionné en haut au préalable.
...
Team Selection
• 3 emplacements d'équipe disponibles (Équipe 1, Équipe 2, Équipe 3)
• Choisir ou modifier l'équipe (clic sur ✏️)
• Toutes les données (joueurs, statistiques, saison, Goal Maps) sont sauvegardées séparément par équipe
...
🥅 Goalie Stats
Sur la page Season, utilise le bouton toggle en haut (« Goalies » / « Players ») pour basculer entre le tableau des patineurs et celui des gardiens. Colonnes gardiens :
• Games — matches avec présence sur la glace
• MIN — minutes jouées (MM:SS)
• GA — Goals Against (buts encaissés)
• SA — Shots Against (tirs cadrés)
• SV — Saves (SA − GA)
• Sv% — Save Percentage (SV ÷ SA ; élite ≥ 92%)
• GAA — Goals Against Average (buts encaissés par 60 min ; élite ≤ 2.00)
• SO — Shutouts (matches sans but encaissé)
• Goal Value — pondération inversée des adversaires : les buts contre des adversaires faibles pèsent davantage
• MVP / MVP Points — classement des gardiens (Sv% × temps de glace − Goal Value/match + bonus de titularisation)

Lors de l'« Export Season », tu seras invité(e) à saisir le temps de glace de chaque gardien (MM:SS ; 0 ou vide = n'a pas joué). Les valeurs sont calculées automatiquement à partir de tes événements Goal Map.
...
• Une sauvegarde contient les 3 équipes à la fois — inutile de sauvegarder équipe par équipe.
...
Reset (irréversible)
• Chaque page dispose de son propre bouton de réinitialisation — il n'y a pas de réinitialisation globale :
• Game Center : réinitialise les statistiques et les temps de glace de l'équipe actuelle
• Season : réinitialise toutes les données de la saison (patineurs + gardiens) de l'équipe actuelle
• Goal Map : réinitialise les points du match actuel (Season Map reste inchangée)
• Season Map : réinitialise les points cumulés de l'équipe actuelle
• Goal Value : réinitialise tous les noms d'adversaires et pondérations de l'équipe actuelle
• Player Selection : réinitialise la liste des joueurs et les gardiens de l'équipe actuelle`
    };
